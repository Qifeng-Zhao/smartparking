...
def getframe_yolo():
    filename = "rtsp://admin：Jingdongyun@192.168.1.64:554/h264/ch1/main/av_stream"
    save_address = "./temprate"
    print("yolobegins")
    ...
    frame = next(frames)
    frame.to_image().save(save_address + "/frame1.jpg")  # % frame.index)

    for ii in range(slot_number):

        img = cv2.imread(save_address + "/frame1.jpg")
        cropped = img[coord[ii][0]:coord[ii][1], coord[ii][2]:coord[ii][3]]
        cv2.imwrite(save_address + "/cropped.jpg", cropped)

        ff, red = detect(weights='yolov5s.pt',  # model.pt path(s)
                         source=save_address + '/cropped.jpg',  # file/dir/URL/glob, 0 for webcam
        ...
        if red != [0, 0]:
            ...
            for jj in range(0, red_num):
                if red[jj][0] > 0.5 and red[jj][1] > 0.5:
                    st = 1
                    print('st=1')
                    break
    ...
    while (1):
        ...
        while k != 10:
            frame = next(frames)
            k = k + 1
        frame.to_image().save(save_address + '/frame2.jpg')  # % frame.index)

        file_path1 = save_address + '/frame1.jpg'
        file_path2 = save_address + '/frame2.jpg'
        for i in range(slot_number):
            coord1 = coord[i]  # ymin ymax xmin xmax
            receive[i] = occupation(file_path1, file_path2, coord1, threshold=0.70)
        ...
        for ii in range(slot_number):
            if flag[ii] == 1:
                ...
                if current_time - start_time[ii] > datetime.timedelta(seconds=3):
                    img = cv2.imread(save_address + "/frame2.jpg")
                    cropped = img[coord[ii][0]:coord[ii][1], coord[ii][2]:coord[ii][3]]
                    cv2.imwrite(save_address + "/cropped.jpg", cropped)
                    ...
                    cursor.execute(sql)
                    conn.commit()
                    sql = ("select local_id,gloable_id,occupation,camera,start_time from id where local_id =" + str(
                        iii))
                    cursor.execute(sql)
                    results = cursor.fetchone()
                    print(results)
